...
func convertLayoutToOciSif(layoutDir string, digest ggcrv1.Hash, imageDest, workDir string) error {
	...
	// Skip AUFS -> OverlayFS whiteout conversion as there are should be no
	// whiteout markers after squashing to single layer.
	squashfsLayer, err := mutate.SquashfsLayer(layers[0], workDir, mutate.OptSquashfsSkipWhiteoutConversion(true))
	if err != nil {
